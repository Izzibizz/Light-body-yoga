      ...
      <div className=" flex flex-col tablet:flex-row tablet:justify-between gap-12 laptop:gap-0">
        <div className="flex flex-col gap-8 justify-end">
         <div className="flex gap-4"> <img src="https://res.cloudinary.com/dbf8xygxz/image/upload/v1739544817/birth_iohmnn.svg" alt="symbol birth" className="w-[40px]"/> <h3 className="font-dream text-3xl">Who am I?</h3></div>
          <p className="laptop:w-10/12 text-justify"><span className="text-xl">Hi I’m Therese!</span> My journey to yoga, and back to embodying my healer’s soul in this lifetime, has been a winding, rocky and beautiful road that’s carried me across the world—a path of movement, creativity, and deep healing, paved with self-discovery and growth. </p>
         <button className="flex items-center gap-2 border border-black rounded-2xl py-1 px-3 w-fit self-center tablet:self-start cursor-pointer hover:scale-105 hover:drop-shadow-2xl hover:bg-warm-white/60 hover:border-warm-white/60" onClick={() => navigate("/about")}>Read more</button>
        </div>
        ...
        /><a href="https://www.studiomamama.se/" target="_blank" rel="noopener noreferrer" className="text-[10px] flex items-center justify-end"><MdOutlineArrowOutward />Photographer: Studio Mamama</a>
        </div>
      </div>
    </section>
  );
